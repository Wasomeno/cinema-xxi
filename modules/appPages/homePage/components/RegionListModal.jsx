...
const RegionListModal = ({ setSelectedRegion, toggleShowRegionList }) => {
  ...
  return (
    <>
      ...
      />
      <AnimatedContainer className="fixed bottom-0 z-20 h-96 w-full rounded-t-lg bg-slate-800 p-4 lg:top-1/2 lg:left-1/2 lg:h-4/6 lg:w-4/6 lg:-translate-x-1/2 lg:-translate-y-1/2 lg:rounded-lg">
        <div className="my-4 text-center">
          <p className="font-poppins text-sm text-white">Region List</p>
        </div>
        <DataContainer
        ...
        >
          {allRegion.data?.map((region) => (
            <button
              ...
              className="w-5/6 rounded-md bg-gray-700 p-3 font-poppins text-xs text-white"
            >
              {region.name}
            </button>
          ))}
